...
def is_possible(value, numbers):
  match numbers:
    ...
    case (*left, last):
      if value % last != 0:
        return is_possible(value - last, tuple(left))
      return is_possible(value - last, tuple(left)) or is_possible(value // last, tuple(left))
...
def is_possible_with_concat(value, numbers):
  match numbers:
    ...
    case (*left, last):
      branches = []
      if value % last == 0:
        branches.append(is_possible_with_concat(value // last, tuple(left)))
      if ends_with(value, last):
        branches.append(is_possible_with_concat(remove_suffix(value, last), tuple(left)))
      if value > last:
        branches.append(is_possible_with_concat(value - last, tuple(left)))
      return any(branches)
...
assert is_possible_with_concat(36, (1,2,3)) == True # 1 || 2 * 3 = 36
assert is_possible_with_concat(103, (5,2,3)) == True # 5 * 2 || 3 = 103
